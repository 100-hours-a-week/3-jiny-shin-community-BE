...
public record UpdatePostRequest(
    @Schema(description = "게시글 제목 (수정하지 않으면 null)", example = "수정된 게시글 제목")
    @Size(min = POST_TITLE_MIN, max = POST_TITLE_MAX, message = "제목은 2자 이상 26자 이하여야 합니다.")
    String title,

    @Schema(description = "게시글 본문 (수정하지 않으면 null)", example = "수정된 게시글 본문입니다.")
    @Size(min = POST_CONTENT_MIN, max = POST_CONTENT_MAX, message = "본문은 2자 이상 10,000자 이하여야 합니다.")
    String content,

    @Schema(description = "첨부 이미지 ID 목록 (수정하지 않으면 null, 최대 5개)", example = "[1, 2]")
    @Size(max = MAX_POST_IMAGES, message = "이미지는 최대 5장까지 첨부할 수 있습니다.")
    List<Long> imageIds,

    @Schema(description = "대표 이미지 ID (imageIds 중 하나를 선택)", example = "1")
    Long primaryImageId
) {
